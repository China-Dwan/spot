...
public class SynchronizedTest {
    ...
    /**
     * 锁升级概念：偏向锁->自旋锁->重量级锁
     * 一个对象刚开始实例化的时候，没人线程来访问，此时是可偏向的，他现在认为只有一个线程能来访问；当第一个线程来访问时，会偏向这个线程，
     * 此时对象持有偏向锁，偏向第一个线程，修改对象头成为偏向锁时使用CAS操作，并将对象头中的线程ID改成自己的id，之后再访问这个对象时就只需要对比id，
     * 不需要进行CAS操作；一旦有第二个线程访问这个对象，因为偏向锁不会主动释放，所以第二个线程可以看到对象是偏向状态，这是表明这个对象上已经存在竞争了，
